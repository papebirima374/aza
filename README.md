...
- Anna Zen Couture en boutique : les 29 modèles sont des lignes « produit » du catalogue (famille `couture`, absente des pages prestations et de la réservation). Prix provisoires de 2 000 à 30 000 F (1 000 F d'écart d'un modèle au suivant, fourchette donnée par Birima) : la direction les corrige ou masque un modèle dans l'écran Catalogue. Page par modèle (`/boutique/couture/C-07`) : taille S à XXL ou sur mesure, « Commander » (panier) ou WhatsApp si la boutique est fermée. Fait sur commande : pas de stock. Contrôles : `node scripts/verifier-couture.mjs`.
