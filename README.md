...
- Carte de fidélité (Plus ▾ → Réglages → 💗 Carte de fidélité, direction ; éteinte par défaut). Par défaut, demande de la gérante : **1 point par passage** (ticket de vente avec téléphone), **cadeau au 10e** : la caisse affiche une carte à tampons, et au seuil un encadré « 🎁 Remettez-lui son cadeau » AVANT la validation ; le ticket part avec `cadeau: true`, `ticket.fidelite.cadeau` est écrit, les points repartent à zéro (`utilises = seuil`), `clientes.cadeauxFidelite` +1. Cadeau décoché = reporté (points gardés). Le cadeau est au choix de l'institut : un soin ou un produit ajouté au ticket avec `offert: true` (0 F, le stock sort). Les commandes en ligne remises comptent aussi (`encaisserCommande`, `cadeau: { remis, texte }`). Autre réglage possible : points selon le montant (`gain: "montant"`) et remise en F (`recompense: "remise"`, bouton « Utiliser N points »). Un avoir reprend tout. Contrôles : `node scripts/verifier-fidelite.mjs`.
- Application installable (PWA) : deux manifestes, `/manifest.webmanifest` (« Anna Zen », le site) et `/gestion/manifest.webmanifest` (« AZA Gestion », scope `/gestion`) ; icônes dans `public/icones` ; service worker `public/sw.js` (pages : réseau d'abord puis dernière version gardée, sinon `/hors-ligne` ; fichiers statiques gardés ; jamais `/api`). Bouton « 📲 Installer » dans Mon compte et en bas du site (Android : fenêtre d'installation ; iPhone : explication Safari). Zoom bloqué comme dans une application (viewport `user-scalable=no`, `touch-action: manipulation`, gestes à deux doigts bloqués pour iPhone, champs en 16 px sur téléphone).
- Accès sur mesure (`lib/acces.ts`) : chaque rôle a ses accès de départ (caisse, remises, clientes, commandes, stock, jour, rapports, avis, journal, catalogue) ; la direction en donne ou en retire à une personne (Équipe → Modifier → 🔐 Accès, champ `comptes.acces`, seules les exceptions au rôle sont gardées). Le serveur vérifie chaque accès (`exigerAcces`) ; Équipe, Réglages, Données et l'agenda restent liés au rôle (règles Firestore inchangées).
- Journal d'activité (`activite/{id}`, `lib/serveur/activite.ts`) : chaque action importante (caisse, rendez-vous, stock, commandes, prix, réglages, équipe et accès, avis, données) est notée par les routes API avec la personne, son rôle et l'heure ; lecture : Tableau de bord → « Qui a fait quoi » (accès « journal »), filtres par personne et par domaine. Jamais modifié ni vidé (hors des « parties » de la remise à zéro). Les Rapports montrent aussi « Qui a encaissé ».
- Clientes : groupe « 🎂 Anniversaires (7 jours) » (champ `anniversaire` « MM-JJ » posé avec la date de naissance ; bandeau sur le tableau de bord du jour), relances WhatsApp déjà écrites (anniversaire, pas venue depuis 3 ou 6 mois, crédit), `derniereRelance` sur la fiche pour ne pas relancer deux fois ; rien n'est envoyé tout seul.
- Ajout d'une prestation en cabine (`POST /api/gestion/rendez-vous/{id}/prestations`) : la praticienne du rendez-vous (ou l'accueil) ajoute un soin ou un produit pendant la séance ou après « J'ai fini », avant l'encaissement ; le rendez-vous est allongé de la durée du soin (occupations ajoutées, verrou du jour), un chevauchement avec le rendez-vous suivant est signalé. Contrôles : `node scripts/verifier-acces-journal.mjs`.
- Ticket de caisse pour imprimante thermique 80 mm (ou 58 mm), noir et blanc, page à la hauteur exacte du ticket (coupe juste après « Merci ») ; réglage par poste (zone imprimable, décalage, taille du texte, avance avant la coupe, logo, QR code) gardé dans le navigateur, avec ticket de réglage (cadre + règle graduée). QR code « Votre avis compte » (bibliothèque `qrcode-generator`).
- Feuille de caisse du jour (`/gestion/caisse/feuille?date=`, bouton dans le bilan de la Caisse) : chacun tire SA feuille (ses tickets, son total, espèces à remettre) ; direction et manager choisissent aussi « Toute la caisse » : ouverture, tickets, totaux, moyens de paiement, qui a encaissé, espèces attendues, compté, écart, clôture, signatures ; « PROVISOIRE » avant la clôture ; sur l'imprimante de tickets (réglage partagé, `components/gestion/Imprimante.tsx`) ou en A4.
- Tableau de bord (direction, manager) : onglets Aujourd'hui · Rapports · Avis clientes ; le comptable a les Rapports. Écrans rares derrière « Plus ▾ » (Catalogue, Collection, Cartes cadeaux, Photos du site, Équipe, Réglages).
- Rapports (`/api/gestion/rapports?du&au`, max. 1 an) : recette, tickets, panier moyen, clientes servies/nouvelles comparés à la période d'avant ; ventes et paiements ; top prestations/produits ; chiffre par praticienne (prestations d'un rendez-vous partagées selon `affectations`) ; rendez-vous et absences ; affluence ; avis. Export Excel des tickets (`&format=csv`, séparateur « ; », BOM UTF-8).
- Avis clientes : lien `/avis/{ticketId}` dans le reçu WhatsApp et QR code du ticket (ventes non annulées, 60 jours) ; un avis par ticket (`avis/{ticketId}`) ; 3 étoiles ou moins « à regarder » (pastille, réponse WhatsApp prête, « Traité ») ; publication sur l'accueil du site par la direction seulement, et seulement si la cliente l'a accepté ; lien « Avis Google » facultatif proposé après 4-5 étoiles. Contrôles : `node scripts/verifier-avis-rapports.mjs`.
